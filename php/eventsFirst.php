...
echo "<tr class='table-primary m-0'><td colspan = '4'><b>" . $day1->format('l, F jS') . "</b></td></tr>";

$eventListSQL = "SELECT * FROM tbl_events WHERE eventTypeID <> 999 AND event_date = '{$day1->format('Y-m-d')}' ORDER BY event_dttm_start ASC";
...
if (!$eventList){
    ...
    while($row = $eventList->fetch_array(MYSQLI_ASSOC))
      {
      echo "<tr class='m-0'>";
      echo "<td style='width: 15%;'>" . date("g:i A", strtotime($row['event_dttm_start'])) . "</td><td class='w-25'>" . $row['event_name'] . "</td><td style='width: 35%;'>" . $row['event_description'] . "</td><td class='w-25'>" . $row['locationID'] . "</td>";
      echo "</tr>";
      }
