...
$facName = htmlspecialchars($_GET['name']);
	...
	$queryApptlst = "SELECT * FROM vw_appt WHERE uniqname = '$facName' ORDER BY appDate, startTime";
	...
	while($items = $resApptlst->fetch_assoc()){
		...
		array_push($resultApptlst, array('apptid' =>$items["apptid"], 'location'=>$items["roomLoc"], 'appTime' =>$appointment->format('g:ia \o\n F jS'), 'studentName' =>$items["studentName"]));
	}
